...
for sp in $samples; do
    ...
    time cat /var3/users_data/KwanwooKim/Metagenome/astha_virome_draft_ver3/5-viral_blastn-CustomCompleteDB/${sp}_blastn_CustomCompleteDB_result.txt |sort -r -nk4 |awk '{if ($4>=50) print ($0)}' > /var3/users_data/KwanwooKim/Metagenome/astha_virome_draft_ver3/5-1_50coverage_blastn/${sp}_blastn_completeDB_result_IF.txt
    time cat /var3/users_data/KwanwooKim/Metagenome/astha_virome_draft_ver3/5-1_50coverage_blastn/${sp}_blastn_completeDB_result_IF.txt |awk -F '\t' '{print $9}'|sort |uniq > /var3/users_data/KwanwooKim/Metagenome/astha_virome_draft_ver3/6-1_50coverage-Taxonomy/${sp}_Custom_completeDB_list.txt
